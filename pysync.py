...
import os,io,sys,math,datetime,random,re,pprint,os.path,zlib,pprint,logging,struct,sqlite3,hashlib,time,signal
import argparse
import SocketServer
import socket
try :
...
class SocketThread( object ):
    ...
    def checkCrcZipSendFile( self, fname , cklist ):
        ...
        if  os.path.getsize(fullname) != cklist[1] :
            g_logger.info( 'size mismatch, %s should %s ' % ( os.path.getsize(fullname) , cklist[1] ) )
            return False
    ...
    def server( self, conninfo):
        ...
        try :
            clientfilelist,clientdeletedlist = serialLoads( ''.join( self.recvData() ) )
        except Exception as e :
            print str(e)
        ...
        syncedfilelist = loadSyncedFileList( self.session.basedir , self.session.conninfo[0])
        ...
        serverupdatelist, clientupdatelist = diffFileList( serverfilelist , clientfilelist )
        ...
        self.sendObject( [serverupdatelist, clientupdatelist,serverdeletedlist] )
        ...
        saveSyncedFileList( serverfilelist , self.session.basedir , self.session.conninfo[0])

        g_logger.info( 'clean up' )
        self.cleanUp()

    def client( self, conninfo ):
        self.session.socket,self.session.conninfo = getClientSocket( *conninfo) , conninfo
        ...
        g_logger.info( 'receiving filelist' )
        try :
            serverupdatelist, clientupdatelist, serverdeletedlist = serialLoads( ''.join(self.recvData() ) )
        except Exception as e :
            print str(e)
            self.errCleanUp('fail to recv/read filelist')
            return
        ...
        saveSyncedFileList( clientfilelist , self.session.basedir , self.session.conninfo[0])
    ...
    def cleanUp(self):
        ...
        g_logger.info( 'recv {totalrecv}:{ziprecv} , sent {totalsent}:{zipsent}'.format( **self.session) )
